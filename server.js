...
MEDIA — про искусство: фильм/кино, книга/литература, картина/живопись, музыка/песня/слова песни, актёр, подкаст, статья, ссылка. Заполни artType ("film"=фильм/кино, "book"=книга/литература, "painting"=картина/живопись, "music"=музыка/песня/текст песни, "other"=прочее) и workTitle (название фильма/книги/картины/песни, если упоминается).
...
async function analyze(text, context = '') {
  ...
  // текущая дата по МСК (UTC+3) — чтобы бот правильно считал относительные даты ("в понедельник")
  const msk = new Date(Date.now() + 3 * 3600 * 1000);
  const todayLine = msk.toLocaleDateString('ru-RU', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric', timeZone: 'UTC' });
  const systemDated = `${SYSTEM}\n\nСЕГОДНЯ: ${todayLine} (часовой пояс Москвы). Для относительных дат мероприятий вычисляй ближайшую будущую дату от этого дня.`;
